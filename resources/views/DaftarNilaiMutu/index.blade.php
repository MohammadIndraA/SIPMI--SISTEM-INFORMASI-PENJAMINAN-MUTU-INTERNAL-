...
@section('title', 'Daftar Nilai Mutu')
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active"> Daftar Nilai Mutu</li>
                    </ol>
                </div>
                <h4 class="page-title"> Daftar Nilai Mutu</h4>
            </div>
        </div>
    </div>
    <div class="row">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-4 col-md-3 d-flex justify-content-end text-end item-button h-50 pt-2">
                            <a href="#" class="btn btn-primary mb-2"
                                onClick="addUser('{{ route('daftar-nilai-mutu.store') }}')"><i
                                    class="mdi mdi-plus-circle me-2"></i>Tambah Daftar Nilai Mutu</a>
                        </div>
                    </div>
    ...
    <x-modal>
        <div class="modal-body">
            ...
            <div class="row mb-1">
                <label for="nilai_mutu" class="col-3 col-form-label">Nilai Nilai <sop class="text-danger">*</sop>
                </label>
                <div class="col-9">
                    <input type="text" class="form-control" name="nilai_mutu" id="nilai_mutu"
                        value="{{ old('nilai_mutu') }}" placeholder="4.0">
                </div>
            </div>
            <div class="row mb-1">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('daftar-nilai-mutu.index') }}",
                    data: function(d) {
        ...
        function addUser(url) {
            ...
            $('#modal-title').text("Tambah Data Daftar Nilai Mutu");
        ...
        function editFunc(id) {
            ...
            $('#myForm').find('.is-invalid').removeClass('is-invalid'); // Remove validation errors
            $('#myForm').find('.invalid-feedback').text(''); // Clear validation error messages
            $('#modal-title').text("Edit Data Daftar Nilai Mutu");
            $('#modal-form').modal('show');
            // url action to update
            let url = `{{ route('daftar-nilai-mutu.update', 'id') }}`
            $('#myForm').attr('action', url.replace('id', id));
            $('#myForm').data('type', 'edit');

            $.ajax({
                type: "GET",
                url: "{{ route('daftar-nilai-mutu.edit') }}",
                data: {
                    id: id
                },
                dataType: 'json',
                success: function(res) {
                    $('#modal-title').html("Edit Data Daftar Nilai Mutu");
                    $('#modal-form').modal('show');
                    $('#id').val(res.data.id);
                    $('#lembaga_akreditasi_id').val(res.data.lembaga_akreditasi_id);
                    $('#fakultas_prodi_id').val(res.data.fakultas_prodi_id);
                    $('#nilai_mutu').val(res.data.nilai_mutu);
        ...
        function deleteFunc(id) {
            if (confirm("Delete Record?") == true) {
                ...
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('daftar-nilai-mutu.delete') }}",
                    data: {
                        id: id
                    },
                    dataType: 'json',
